...
export function DestinationTogglePublish({ id, isPublished }: Props) {
  ...
  function toggle() {
    startTransition(async () => {
      ...
      const { error } = await supabase.from("destinations").update({ is_published: next }).eq("id", id);
      if (error) { toast.error("Failed."); } else { setPublished(next); toast.success(next ? "Published." : "Unpublished."); router.refresh(); }
    });
  }
  return (
    <button onClick={toggle} disabled={pending}
      ...
      role="switch" aria-checked={published} aria-label={published ? "Unpublish" : "Publish"}>
